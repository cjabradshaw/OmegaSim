...
## slowly increase maximum number of assessments per species in region 3
...
R3maxassess.vec <- seq(2,80,1)

omegasSdiff.md <- omegasSdiff.up <- omegasSdiff.lo <- rep(NA, length(R3maxassess.vec))

for (f in 1:length(R3maxassess.vec)) {
  ...
  # region 3  
  assess.limR3 <- c(1, R3assess.vec[f]) # range of number of assessments per species
...
plot(R3maxassess.vec, omegasSdiff.md, type="l", xlab="max assessments/sp", ylab="sum region pairwise diffs",
     ylim=c(min(omegasSdiff.lo), max(omegasSdiff.up)))
lines(R3maxassess.vec, omegasSdiff.lo, lty=2, col="red")
lines(R3maxassess.vec, omegasSdiff.up, lty=2, col="red")


R3maxassess.dat <- data.frame(R3maxassess.vec, omegasSdiff.md, omegasSdiff.up, omegasSdiff.lo)
write.table(R3maxassess.dat, "R3maxassess.csv", row.names=F, sep=",")
