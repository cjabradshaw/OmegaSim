...
## slowly reduce confidence in region 3
...
R3lowconf.vec <- seq(0.05,0.95,0.05)

omegasSdiff.md <- omegasSdiff.up <- omegasSdiff.lo <- rep(NA, length(R3lowconf.vec))

for (f in 1:length(R3lowconf.vec)) {
  ...
  # region 3  
  conf.wR3 <- c(R3lowconf.vec[f],0.5,1-R3lowconf.vec[f]) # confidence probability weights
...
plot(R3lowconf.vec, omegasSdiff.md, type="l", xlab="probability low confidence", ylab="sum region pairwise diffs",
     ylim=c(min(omegasSdiff.lo), max(omegasSdiff.up)))
lines(R3lowconf.vec, omegasSdiff.lo, lty=2, col="red")
lines(R3lowconf.vec, omegasSdiff.up, lty=2, col="red")

R3lowconf.dat <- data.frame(R3lowconf.vec, omegasSdiff.md, omegasSdiff.up, omegasSdiff.lo)
write.table(R3lowconf.dat, "R3lowconf.csv", row.names=F, sep=",")
